...
3. Branch `phase-1-trivia` is CREATED and checked out. Step 1 (shared client + theme) satisfied by Phase 0. **DONE this session:** (a) `game_scoreboard(game_id)` SQL fn — migration `0016` (QUAL-4, wildcard ×2 matches legacy; anon-safe). (b) **`Leaderboard.tsx` PORTED + verified in-browser** — renders through `DisplayCanvas` (portrait, terminal re-skin, not a pixel copy of the legacy light theme) via realtime (ARCH-1: `useLeaderboard.ts`, one channel + 45s global fallback poll, NO 5s polling), QUAL-4 (RPC), QUAL-1 (`shared/log.ts` DEV-gated), PERF-1 (no flicker). New dep `qrcode.react` (local QR, replaces legacy external api.qrserver.com). Verified: FINAL SCORES board (17 real teams, order+wildcard correct), holding screen (QR+countdown+teams), live realtime reorder without reload, `?calibrate`, no console noise. `?game=<id>` override added for testing/History. Test fixture game `fa11face-…-0001` was created+deleted (recreate for full end-to-end).
   ...
   **All 3 read-only display pages DONE.** **NEXT:** host tools — GameSetup → QuestionEntry → BulkImport → VideoEntry (+ edge fns parse-powerpoint, analyze-image — need GEMINI_API_KEY from Stephen); **Scoring decomposition LAST** (docs/04 ARCH-2, parity checklist open). Deferred: bonus-round badges on Leaderboard (per-round detail); History write actions; "Scoring in Progress" interstitial dropped (no `rounds.scoring_in_progress`).
4. Real data in DB — test displays against game `bb00eca3-…-3cad42e` (2026-03-18, 17 teams, wildcards+tiebreakers). Note: default `/leaderboard` resolves to the highest-priority non-completed game (several imported games carry non-`completed` legacy statuses); pass `?game=` to pin one.
5. **Phase 1 GATE still open:** full docs/04 parity checklist run end-to-end on a fresh fake game (needs Scoring/GameDisplay), explicit anon-write RLS rejection test, host+staff runbooks (docs/12). Not started.
