...
async function main(): Promise<void> {
  ...
  log.info(`bunker-agent ${AGENT_VERSION} starting — READ-ONLY mirror (no control changes)`);
  ...
  const client = new QrcClient({ host: config.coreHost, port: config.corePort, log: (lvl, m) => log(lvl, m) });
