...
`npm run dist:win` builds the Windows NSIS installer via electron-builder (config
in `package.json` → `build`). The ffmpeg/ffprobe static binaries are `asarUnpack`-ed
so they're runnable from inside the packaged app. **Don't** build the installer on
macOS for production use — build on/for Windows x64. `npm run pack` produces an
unpacked dir for local inspection.
