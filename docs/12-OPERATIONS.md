...
- **TV-chunk gate (runs after the build, before the deploy):** `node scripts/check-tv-chunk.mjs` (`pnpm test:tvchunk`) walks the built `displayRoutes-*.js` closures — the bar TVs' `/signage/s/:slug` and the legacy `/drinks` — and fails the job if any staff-console chunk or marker reached them, or if a display sentinel went missing. The PR #106 isolation contract is a build gate, not a comment.
